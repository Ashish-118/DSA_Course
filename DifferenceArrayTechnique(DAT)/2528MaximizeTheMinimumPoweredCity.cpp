...
// optimisation T.C : O(n * log(sum+k)), sum = sum of station values S.C : O(n)
class Solution
{
public:
    bool check(long long mid, vector<long long> &diff, int r, int k, int n)
    {
        ...
        long long cumSum =
            0; // it will give power of a city at any point of time
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (cumSum < mid)
            {
                ...
                k -= need;
                cumSum += need; // gredily I had chosen to add station on the
                                // city which can cover more range

                // i......X.....i+2*r
                if (i + 2 * r + 1 < n)
                    tempDiff[i + 2 * r + 1] -=
                        need; // applying difference array technique
            }
        }
